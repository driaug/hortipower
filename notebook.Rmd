...
```{r}
ggplot(tomaat) + geom_point(mapping = aes(x = time, y = Sapstroom), size=0.1, alpha=0.1) + geom_smooth() + geom_point(mapping = aes(x = time, y = Diameter), size=0.1, alpha=0.1) + geom_smooth()
```
```

```{r}
ggplot(tomaat, mapping = aes(x = date, y = Diameter)) + geom_point(size=0.1, alpha=0.1) + geom_smooth()
```
