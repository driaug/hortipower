...
```{r}
ggplot(tomaat, mapping = aes(x = time, y = Sapstroom)) + geom_point(size=0.1, alpha=0.1) + geom_smooth(method = "lm")
```
...
```{r}
ggplot(data, mapping = aes(x = date, y = buitentemperatuur)) + geom_point(size=0.1, alpha=0.1) + geom_smooth()
```