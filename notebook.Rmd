...
HPS <- HPS %>% mutate(DateTime=as.POSIXct(DateTime, format = "%d/%m/%Y %H:%M"))
```
...
LED <- LED %>% mutate(DateTime=as.POSIXct(DateTime, format = "%d/%m/%Y %H:%M"))
```
...
```{r}
ggplot(tomaat) + geom_col(mapping = aes(x = DateTime, y = Sapstroom))
```